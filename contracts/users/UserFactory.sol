...
contract UserFactory is Ownable {
  ...
  event BasicUserCreated(address owner, address userAddress);
  event OrganizerCreated(address owner, address userAddress);

  function UserFactory(address _BSTokenFrontend, address _EventFactory) public Ownable(msg.sender) {
  ...
  function createBasicUser() public returns (address) {
    ...
    BasicUserCreated(msg.sender, basicUser);
    return basicUser;
  }
  ...
  function createOrganizer() public returns (address) {
    ...
    usedAddresses[msg.sender] = true;
    OrganizerCreated(msg.sender, organizer);
    return organizer;
  }
